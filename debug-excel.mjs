...
const debugExcelFile = async (filePath) => {
  try {
    ...
    console.log(`  Dimensions: ${ws.dimensions ? ws.dimensions.ref : "unknown"}`);
    ...
    const keywords = ["LIQUID", "PLASTIC", "SHRINK", "PENETRATION", "LIMIT", "TRIAL"];
    for (let row = 1; row <= Math.min(100, ws.actualRowCount || 100); row++) {
      for (let col = 1; col <= Math.min(20, ws.actualColumnCount || 20); col++) {
        const cell = ws.getCell(row, col);
